...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  const distanceButton = document.getElementById('get-distance')
  const grabButton = document.getElementById('claw-grab')
  const releaseButton = document.getElementById('claw-release')

  distanceButton.onclick = () => {
    const query = `${baseHost}/command?action=distance`

    fetch(query)
        .then(r => r.text())
        .then(data => logResponse("Distance: " + data))
        .catch(e => logResponse("Distance: Error"));
  }

  grabButton.onclick = () => {
    const query = `${baseHost}/command?action=grab`

    fetch(query)
        .then(r => r.text())
        .then(data => logResponse("Grab: " + data))
        .catch(e => logResponse("Grab: Error"));
  }

  releaseButton.onclick = () => {
    const query = `${baseHost}/command?action=release`

    fetch(query)
        .then(r => r.text())
        .then(data => logResponse("Release: " + data))
        .catch(e => logResponse("Release: Error"));
  }
